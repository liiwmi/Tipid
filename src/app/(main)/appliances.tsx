...
export default function AppliancesScreen() {
  ...
  const { appliances, activeAppliances, loading, isOnline, addAppliance } = useAppliances();
  const { addApplianceVisible, openAddAppliance, closeAddAppliance } = useAppContext();
  const insets = useSafeAreaInsets();


  return (
    <SafeAreaView edges={['top', 'bottom', 'left', 'right']} style={[screenStyles.container, { backgroundColor: colors.bgSecondary }]}>

      {/* HEADER */}
      <View style={[screenStyles.header, { borderBottomColor: colors.borderDefault }]}>
        <Text style={[screenStyles.title, { color: colors.textPrimary }]}>Appliances</Text>
        <View style={screenStyles.headerRight}>
          {!isOnline && (
            <View style={[screenStyles.offlineBadge, { backgroundColor: colors.priorityHighBg }]}>
              <Ionicons name="cloud-offline-outline" size={12} color={colors.priorityHighText} />
              <Text style={[screenStyles.offlineText, { color: colors.priorityHighText }]}>
                Offline
              </Text>
            </View>
          )}
          <TouchableOpacity
            style={[screenStyles.addButton, { backgroundColor: colors.primary }]}
      ...
      <View style={[screenStyles.statsRow, { backgroundColor: colors.bgCard }]}>
        ...
        <View style={[screenStyles.statDivider, { backgroundColor: colors.borderDefault }]} />
        <View style={screenStyles.statItem}>
        ...
        <View style={[screenStyles.statDivider, { backgroundColor: colors.borderDefault }]} />
        <View style={screenStyles.statItem}>
          <Text style={[screenStyles.statValue, { color: colors.priorityHighText }]}>
        ...
        <View style={[screenStyles.statDivider, { backgroundColor: colors.borderDefault }]} />
        <View style={screenStyles.statItem}>
          <Text style={[screenStyles.statValue, { color: colors.priorityMedText }]}>
      ...
      {/* FAB */}
      <TouchableOpacity
        style={[screenStyles.fab, { backgroundColor: colors.primary, bottom: 30 + insets.bottom }]}
        onPress={() => openAddAppliance()}
      >
