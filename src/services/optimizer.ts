...
function applyPriorityScaling(
  ...
  return items.map((item, i) => {
    ...
    return {
      ...item,
      priority: item.priority + (inPeak ? 5 : 0),
    };
  });
}
...
function solveKnapsack(
  ...
  // Filter zero-cost items to avoid division by zero
  const validItems = items.filter((item) => item.cost > 0);
  ...
  while (queue.length > 0) {
    ...
    if (u.bound > maxProfit && u.level < n - 1) {
      ...
      // BRANCH 1: include this appliance
      const vInclude: Node = {
        level: nextLevel,
      ...
      if (vInclude.weight <= budget && vInclude.profit > maxProfit) {
        maxProfit = vInclude.profit;
        bestSelected = vInclude.selectedItems;
      }

      vInclude.bound = calculateBound(vInclude, n, budget, validItems);
      if (vInclude.bound > maxProfit) queue.push(vInclude);

      // BRANCH 2: exclude this appliance
      const vExclude: Node = {
        level: nextLevel,
        profit: u.profit,
        weight: u.weight,
      ...
      if (vExclude.bound > maxProfit) queue.push(vExclude);
    }
  }
...
export function runOptimization(
  ...
  // Only optimize active appliances
